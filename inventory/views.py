...
def member_mtc(request):
    members = MemberMTC.objects.all()

    return render(request, 'member_mtc.html', {
        'members': members
    })
...
def tambah_member(request):
    if request.method == 'POST':
        form = MemberMTCForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('member_mtc')
    else:
        form = MemberMTCForm()
...
def edit_member(request, pk):
    member = get_object_or_404(MemberMTC, pk=pk)
    ...
    if request.method == 'POST':
        ...
        form = MemberMTCForm(instance=member)
...
def delete_member(request, pk):
    member = get_object_or_404(MemberMTC, id=pk)
    member.delete()
...
def kaizen_list(request):

    data = Kaizen.objects.all().order_by('-tanggal')
...
def kaizen_create(request):
    ...
    return render(
        ...
        {
            'form': form,
            'judul': 'Tambah Schedule Kaizen'
        }
    )
def kaizen_list(request):
    data = Kaizen.objects.all().order_by('-id')
    return render(request, 'kaizen_list.html', {
        'data': data
    })
def kaizen_edit(request, pk):
    kaizen = Kaizen.objects.get(id=pk)
    form = KaizenForm(instance=kaizen)

    if request.method == 'POST':
        form = KaizenForm(request.POST, request.FILES, instance=kaizen)
        if form.is_valid():
            form.save()
            return redirect('kaizen_list')

    return render(request, 'kaizen_form.html', {
        'form': form,
        'judul': 'Edit Kaizen'
    })
def kaizen_delete(request, pk):
    kaizen = Kaizen.objects.get(id=pk)
    ...
    return render(request, 'kaizen_delete.html', {
        'data': kaizen
    })
def kaizen_detail(request, pk):
    kaizen = Kaizen.objects.get(id=pk)
